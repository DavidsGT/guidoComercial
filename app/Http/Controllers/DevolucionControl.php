...
class DevolucionControl extends Controller
{
    ...
    public function index(Request $request)
    {
    		...
    		if ($request)
    		{
    			...
    			->select('i.idingreso','i.fecha_hora','p.nombre','i.tipo_comprobante','i.serie_comprobante','i.numero_comprobante','i.impuesto','i.estado',DB::raw('(i.subtotal+i.impuesto) as total'),DB::raw('sum(di.cantidad) as cantidad'),'i.formapago')
				...
				return view('compras.devoluciones.index',["ingresos"=>$ingreso,"searchText"=>$query]);
    		}
    }
    ...
    public function create($ing)
    {
        ...
        ->select('a.idarticulo','a.nombre as articulo','d.cantidad','d.precio_compra','d.precio_venta','d.tipoiva')
        ->where('d.idingreso','=',$ing)
        ->get();

        return view('compras.devoluciones.create',['cuentas'=>$cuenta,'ordenes'=>$orden,'detallecp'=>$detallecp,'ingreso'=>$ingreso,'detalles'=>$detalle]);
    }
...
//alamcenar en la base
     ...
     public function show($ing)
    {
       ...
       $compegre = DB::table('devolucion as dc')
        ...
        return view('compras.devoluciones.show',['compegres'=>$compegre,'cuentas'=>$plan,'detart'=>$detalle]);
    }
